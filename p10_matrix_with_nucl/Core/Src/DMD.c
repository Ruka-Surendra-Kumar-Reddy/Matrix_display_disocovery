...
uint8_t bDMDScreenRAM[64]; //need to change back to pointer
const uint8_t* Font;
...
void clearScreen(uint8_t bNormal)
{
    if (bNormal) // clear all pixels
        memset(bDMDScreenRAM,0x0F,DMD_RAM_SIZE_BYTES*DisplaysTotal);
    else // set all pixels
...
//    bDMDScreenRAM = (uint8_t *) malloc(DisplaysTotal*DMD_RAM_SIZE_BYTES);
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_RESET); //SCK
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET); //MOSI
